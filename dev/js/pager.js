...
const _pager = ({total, subtotal, queryString, page, size, fp, fs, resource, subresource, htmlElement}) => {
    ...
    outputPager({
        page_list, queryString, page, size, fp, fs, htmlElement, resource
    })
}
...
const pager = ({total, subtotal, queryString, page, size, fp, fs, resource, subresource, htmlElement}) => {
    ...
    if (resource === 'treatments' && (total > 0 && subtotal > 0)) {
        ...
        sel_pageLinks.forEach(p => {
            p.addEventListener('click', showFig)
        })
    }
}

const outputPager = ({page_list, queryString, page, size, fp, fs, htmlElement, resource}) => {
