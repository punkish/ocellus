...
"undefined"!=typeof O&&"object"==typeof O||(O={}),O.base={"num-of-records":{images:0,treatments:0,citations:0},logger:function(e,o){const s="-".repeat(50);log.info(s),log.info(`${e}: ${o}`),log.info(s)},init:function(){const e=["treatments","citations","images"],o=["about","ip","contact","privacy"];O.base.addEventHandlers([{selectors:document.querySelectorAll(".modal-open"),action:"click",handler:{name:"openModal()",fn:O.openModal}},{selectors:document.querySelectorAll(".modal-close"),action:"click",handler:{name:"closeModal()",fn:O.closeModal}}]);const{hash:s,resource:t,search:n}=O.base.resourceFromUrl(location);if(O.closedForMaintenance){"index"===t||e.includes(t)?O.base.show("closedForMaintenance"):o.includes(t)&&O.base.show(t)}else O.base.addEventHandlers([{selectors:[document.querySelector("#searcher")],action:"submit",handler:{name:"getResource()",fn:O.utils.getResource}},{selectors:[document.querySelector("input[name=submit]")],action:"click",handler:{name:"getResource()",fn:O.utils.getResource}},{selectors:document.querySelectorAll("input[name=resource]"),action:"click",handler:{name:"selectResource()",fn:O.base.selectResource}},{selectors:[document.querySelector("input[name=refreshCache]"),document.querySelector("input[name=communitiesChooser]")],action:"click",handler:{name:"toggler()",fn:O.base.toggler}}]),o.includes(t)?O.base.show(t):(O.base.show("searcher"),O.base.logger("init()","initializing autocomplete"),new autoComplete({selector:document.querySelector("input[name=q]"),minChars:3,source:function(e,o){try{fetch.abort()}catch(e){}fetch(`${zenodeoUri}/families?q=${e}`).then(O.fetchReceive).then(o)}}),log.info("- getting "+t),"index"===t?(log.info("- getting selected resource"),O.base.setP(document.querySelector("input[name=resource]:checked").value),O.base.show("index")):e.includes(t)&&(n?(log.info("- getting search"),O.base.fillSearchForm(n,t)):(log.info("- getting "+t),O.base._selectResource(t),O.base.setP(t),O.base.show(t))))},addEventHandlers:function(e){O.base.logger("addEventHandlers()","adding event handlers");for(let o=0,s=e.length;o<s;o++){const{selectors:s,action:t,handler:n}=e[o];for(let e=0,o=s.length;e<o;e++){const o=s[e],c=o.name||o.id||`${o.nodeName}.${o.pathname.split("/").pop()}`;log.info(`- added ${n.name} to ${c}`),o.addEventListener(t,n.fn)}}},toggler:function(e){const o=e.target.name,s=document.querySelector("#"+o);s.classList.contains("hidden-none")?(s.classList.add("visible-block"),s.classList.remove("hidden-none")):s.classList.contains("hidden-block")?(s.classList.add("visible-block"),s.classList.remove("hidden-block")):s.classList.contains("visible-block")&&(s.classList.add("hidden-block"),s.classList.remove("visible-block"))},openModal:function(e){const o=document.querySelectorAll("main section");for(let e=0,s=o.length;e<s;e++)O.base.hide(o[e].id);const s=e.target.href.split("/").pop().split(".").shift(),t=s+".html";history.pushState(null,null,t),O.base.show(s),e.stopPropagation(),e.preventDefault()},closeModal:function(e){let o=e.target.parentElement.id;O.base.hide(o),O.closedForMaintenance?o="closedForMaintenance":(o="index",O.base.show("searcher"));const s=O.closedForMaintenance?"index.html":o+".html";history.pushState(null,null,s),O.base.show(o),e.stopPropagation(),e.preventDefault()},show:function(e){log.info("showing "+e);const o=document.querySelector("#"+e);o.classList.add("visible-block"),o.classList.remove("hidden-none")},hide:function(e){log.info("hiding "+e);const o=document.querySelector("#"+e);o.classList.add("hidden-none"),o.classList.remove("visible-block")},formatSearchCriteria:function(e){const o=["resource","communities","communitiesChooser","refreshCache","size","page","reset","submit"],s=[];for(let t in e)if(!o.includes(t)){const o=e[t];"q"===t?32===e[t].length?"treatments"===e.resource?s.push(`<span class="crit-key">treatmentId</span> is <span class="crit-val">${o}</span>`):"citations"===e.resource&&s.push(`<span class="crit-key">bibRefCitationId</span> is <span class="crit-val">${o}</span>`):s.push(`<span class="crit-val">${o}</span> is in the text`):s.push(`<span class="crit-key">${t}</span> is <span class="crit-val">${o}</span>`)}let t=s.length,n="";return 1===t?n+=s[0]:2===t?n+=s.join(" and "):t>2&&(t-=1,n+=s.slice(0,t).join(", ")+", and "+s[t]),n},toggleFigcaption:function(e){const o=document.querySelectorAll("figcaption");for(let e=0,s=o.length;e<s;e++)o[e].style.maxHeight="30px",o[e].style.overflow="hidden";const s=this.parentElement;s.style.maxHeight="100%",s.style.overflow="auto"},resourceFromUrl:function(e){const{hash:o,pathname:s,search:t}=e;O.base.logger("resourceFromUri()",e);let n=s.split("/").pop().split(".")[0];return""!==n&&"dev"!==n||(n="index"),log.info("- hash: "+o),log.info("- resource: "+n),log.info("- search: "+t),{hash:o,resource:n,search:t}},resourceFromForm:function(e){return document.querySelector("input[name=resource]:checked").value},resourceFromModalClick:function(e){const o=e.target.href;if(O.logger("resourceFromUri()",loc),log.info("- href: "+o),o)return o.split("/").pop().split(".").shift()},_selectResource:function(e){const o=document.querySelectorAll("#resourceChooser label"),s=document.querySelectorAll("#resourceChooser input");for(let t=0,n=o.length;t<n;t++)s[t].value===e?(s[t].checked=!0,o[t].classList.add("searchfocus")):(s[t].checked=!1,o[t].classList.remove("searchfocus"))},selectResource:function(e){const o=e.target.value;log.info("selected resource "+o),O.base._selectResource(o),O.base.setP(o)},setP:function(e){O.base.logger("setP()","setting placeholder"),O.base["num-of-records"][e]?O.base.setPlaceHolder({"num-of-records":O.base["num-of-records"][e],resource:e}):O.utils.getBar({resource:e,inputs:null,cb:O.base.setPlaceHolder})},setPlaceHolder:function(e){0==O.base["num-of-records"][e.resource]&&(O.base["num-of-records"][e.resource]=e["num-of-records"]);const o=`search ${e["num-of-records"]} ${e.resource}`;log.info(`placeholder: "${o}"`);const s=document.querySelector("input[name=q]");s.placeholder=o,s.focus()},fetchReceive:function(e){if(!e.ok)throw new Error("HTTP error, status = "+e.status);return e.json()},hls:function(e){const o=document.querySelectorAll("input[name=resource");for(let s=0,t=o.length;s<t;s++){const t=o[s],n=t.parentElement;t.value===e?(log.info(`setting checked of ${t.value} to true`),t.checked=!0,n.classList.add("searchfocus")):(t.checked=!1,n.classList.remove("searchfocus"))}},fillSearchForm:function(e,o){O.base.logger("fillSearchForm()","filling search form"),log.info("- search: "+e),log.info("- resource: "+o);const s=["communities","page","size","q","doi","author","text"],t=document.querySelector("#searcher"),n=new URLSearchParams(e);for(const[e,o]of n)if(s.includes(e)){const s=t.querySelectorAll(`input[name=${e}]`);for(let e=0,t=s.length;e<t;e++){const t=s[e];"checkbox"===t.type?t.value===o&&(log.info(`setting checked of ${t.name} with value ${t.value} to 'true'`),t.checked=!0):t.value=o}}O.base.hls(o),O.utils.getResource()}};
"undefined"!=typeof O&&"object"==typeof O||(O={}),O.utils={niceNumbers:function(e){return e>9?e:["No","One","Two","Three","Four","Five","Six","Seven","Eight","Nine"][e]},renderResults:function(e){const t=e.resource,s=e.template,r=document.querySelector(`#${t} div.result`),n=document.querySelector("#template-"+s).innerHTML,i={"template-pager":document.querySelector("#template-pager").innerHTML,"template-records-found":document.querySelector("#template-records-found").innerHTML,"template-kein-preview":document.querySelector("#template-kein-preview").innerHTML};r.innerHTML=Mustache.render(n,e,i);const o=document.querySelectorAll("figcaption > a");for(let e=0,t=o.length;e<t;e++)o[e].addEventListener("click",O.base.toggleFigcaption);return e.resource},getResource:function(e){e&&(e.stopPropagation(),e.preventDefault());const t=document.querySelector("#searcher"),s=t.elements,r=t.querySelector("input[name=q]");if(""===r.value)return r.placeholder="c‘mon, enter something",r.classList.remove("nudge"),r.classList.add("warning"),r.focus(),!1;r.classList.remove("warning"),r.classList.add("nudge");const n={};for(let e=0,r=s.length;e<r;e++){const s=t[e];if(s.name){const e=s.name,t=s.value,r=s.type,i=s.checked;"checkbox"===r?i&&(e in n?n[e].push(t):n[e]=[t]):"radio"===r?i&&(n[e]=t):n[e]=t}}O.utils.getFoo({resource:document.querySelector("input[name=resource]:checked").value,inputs:n,cb:O.utils.renderResults})},noThrob:function(e){const t=document.querySelectorAll("main section");for(let s=0,r=t.length;s<r;s++){const r=t[s].id;r!==e&&O.base.hide(r)}document.querySelector("#throbber").classList.add("nothrob")},getBar:function(e){const{resource:t,inputs:s,cb:r}=e,{z:n,b:i}=O.utils.inputs2uris(s,t);O.base.logger("getBar()","getting "+t),log.info("zenodeoUri: "+n),log.info("browserUri: "+i);let o;document.querySelector("#throbber").classList.remove("nothrob"),s&&(o=s?O.base.formatSearchCriteria(s):"",O.utils.waitMessage(t,o),history.pushState(null,null,i),O.base.show(t));try{fetch(n).then(O.base.fetchReceive).then((function(e){const r="value"in e?e.value:e,n=r["num-of-records"],c={resource:t,"num-of-records":n};if(s){const e=30;c["search-criteria-text"]=o,32===s.q.length?("treatments"===r["search-criteria"].resource?c.template="treatment":"citations"===r["search-criteria"].resource&&(c.template="citation"),n&&(c.figures=r.records[0])):(c.template=c.resource,c.niceNumbers=O.utils.niceNumbers(n),n?(c.successful=!0,c.shown=1==n?"it is shown below":n>1&&n<=e?`${O.utils.niceNumbers(r.from)} to ${O.utils.niceNumbers(n)} are shown below`:`${O.utils.niceNumbers(r.from)} to ${O.utils.niceNumbers(r.to)} are shown below`,c.figures=function(e,t){let s=[];if("treatments"===e)for(let e=0,r=t.length;e<r;e++){const r=t[e];r.recId=r.treatmentId,s.push(r)}else if("images"===e)for(let e=0,r=t.length;e<r;e++){const r=t[e];s.push({title:r.metadata.title,creators:r.metadata.creators?r.metadata.creators.map((e=>e.name)):[],recId:r.id,zenodoRecord:`${zenodoUri}/${r.id}`,description:r.metadata.description,doi:r.doi,img:!!r.links.thumbs,img10:r.links.thumbs?r.links.thumbs[10]:"",img50:r.links.thumbs?r.links.thumbs[50]:"",img100:r.links.thumbs?r.links.thumbs[100]:"",img250:r.links.thumbs?r.links.thumbs[250]:"",img750:r.links.thumbs?r.links.thumbs[750]:"",img1200:r.links.thumbs?r.links.thumbs[1200]:""})}else if("citations"===e)for(let e=0,r=t.length;e<r;e++){const r=t[e];r.recId=r.bibRefCitationId,s.push(r)}return s}(t,r.records),n>=e&&(c.prev=i.replace(/page=\d+/,"page="+r.prevpage),c.next=i.replace(/page=\d+/,"page="+r.nextpage),c.pager=!0)):(c.successful=!1,c.shown="",c.pager=!1,c.figures=[]))}return c})).then(r).then(O.utils.noThrob)}catch(e){log.error(e)}},getFoo:function(e){const{resource:t,inputs:s,cb:r}=e,{z:n,b:i}=O.utils.inputs2uris(s,t);O.base.logger("getFoo()","getting "+t),log.info("zenodeoUri: "+n),log.info("browserUri: "+i);let o;document.querySelector("#throbber").classList.remove("nothrob"),s&&(o=s?O.base.formatSearchCriteria(s):"",O.utils.waitMessage(t,o),history.pushState(null,null,i),O.base.show(t));try{fetch(n).then(O.base.fetchReceive).then((function(e){const r="value"in e?e.value:e,n=r["num-of-records"],c={resource:t,"num-of-records":n};if(s){const e=30;c["search-criteria-text"]=o,32===s.q.length?("treatments"===r["search-criteria"].resource?c.template="treatment":"citations"===r["search-criteria"].resource&&(c.template="citation"),n&&(c.figures=r.records[0])):(c.template=c.resource,c.niceNumbers=O.utils.niceNumbers(n),n?(c.successful=!0,c.shown=1==n?"it is shown below":n>1&&n<=e?`${O.utils.niceNumbers(r.from)} to ${O.utils.niceNumbers(n)} are shown below`:`${O.utils.niceNumbers(r.from)} to ${O.utils.niceNumbers(r.to)} are shown below`,c.figures=function(e,t){let s=[];if("treatments"===e)for(let e=0,r=t.length;e<r;e++){const r=t[e];r.recId=r.treatmentId,s.push(r)}else if("images"===e)for(let e=0,r=t.length;e<r;e++){const r=t[e];s.push({title:r.metadata.title,creators:r.metadata.creators?r.metadata.creators.map((e=>e.name)):[],recId:r.id,zenodoRecord:`${zenodoUri}/${r.id}`,description:r.metadata.description,doi:r.doi,img:!!r.links.thumbs,img10:r.links.thumbs?r.links.thumbs[10]:"",img50:r.links.thumbs?r.links.thumbs[50]:"",img100:r.links.thumbs?r.links.thumbs[100]:"",img250:r.links.thumbs?r.links.thumbs[250]:"",img750:r.links.thumbs?r.links.thumbs[750]:"",img1200:r.links.thumbs?r.links.thumbs[1200]:""})}else if("citations"===e)for(let e=0,r=t.length;e<r;e++){const r=t[e];r.recId=r.bibRefCitationId,s.push(r)}return s}(t,r.records),n>=e&&(c.prev=i.replace(/page=\d+/,"page="+r.prevpage),c.next=i.replace(/page=\d+/,"page="+r.nextpage),c.pager=!0)):(c.successful=!1,c.shown="",c.pager=!1,c.figures=[]))}return c})).then(r).then(O.utils.noThrob)}catch(e){log.error(e)}},waitMessage:function(e,t){const s=document.querySelector("#"+e);O.base.show(e);s.querySelector(".result").innerHTML=`<p id="records-found" class="records-found">Looking for ${e} where ${t}</p>`},inputs2uris:function(e,t){const s={images:["q","size","page","communities","refreshCache"],treatments:["q","size","page","refreshCache"],citations:["q","size","page","refreshCache"]},r={images:["q","size","page","communities"],treatments:["q","size","page"],citations:["q","size","page"]},n=[],i=[];for(const o in e)s[t].includes(o)&&("q"===o?32==o.length?"treaments"===t?n.push("treatmentId="+e[o]):"citations"===t&&n.push("bibRefCitationId="+e[o]):n.push("q="+e[o]):n.push(`${o}=${e[o]}`)),r[t].includes(o)&&i.push(`${o}=${e[o]}`);const o=n.length?"?"+n.sort().join("&"):"";return{z:`${zenodeoUri}/${t}${o}`,b:`${t}.html${i.length?"?"+i.join("&"):""}`}}};