...
"undefined"!=typeof O&&"object"==typeof O||(O={}),O.base={"num-of-records":{images:0,treatments:0,citations:0},logger:function(e,o){const s="-".repeat(50);log.info(s),log.info(`${e}: ${o}`),log.info(s)},init:function(){log.level=log[O.globals.loglevel],log.info("the log level is "+O.globals.loglevel),O.base.addEventHandlers([{selectors:document.querySelectorAll(".modal-open"),action:"click",handler:{name:"openModal()",fn:O.openModal}},{selectors:document.querySelectorAll(".modal-close"),action:"click",handler:{name:"closeModal()",fn:O.closeModal}}]);const{hash:e,resource:o,search:s}=O.base.resourceFromUrl(location);O.closedForMaintenance?O.globals.pseudoResources.includes(o)?O.base.show(o):O.base.show("closedForMaintenance"):(O.base.addEventHandlers([{selectors:[document.querySelector("#searcher")],action:"submit",handler:{name:"prepareQuery()",fn:O.utils.prepareQuery}},{selectors:[document.querySelector("input[name=submit]")],action:"click",handler:{name:"prepareQuery()",fn:O.utils.prepareQuery}},{selectors:document.querySelectorAll("input[name=resource]"),action:"click",handler:{name:"selectResource()",fn:O.base.selectResource}},{selectors:[document.querySelector('input[name="refreshCache"]'),document.querySelector("input[name=communitiesChooser]")],action:"click",handler:{name:"toggler()",fn:O.base.toggler}}]),O.globals.pseudoResources.includes(o)?O.base.show(o):(O.base.show("searcher"),O.base.logger("init()","initializing autocomplete"),new autoComplete({selector:document.querySelector("input[name=q]"),minChars:3,source:async function(e,o){try{fetch.abort()}catch(e){}let s=await fetch(`${O.globals.zenodeoUri}/families?q=${e}`);if(s.ok){const e=await s.json();if(e.item.result.count){o(e.item.result.records.map((e=>e.family)))}}else alert("HTTP-Error: "+s.status)}}),log.info("- getting "+o),"index"===o?(log.info("- getting selected resource"),O.base.updatePlaceholder(document.querySelector("input[name=resource]:checked").value),O.base.show("index")):O.globals.resources.includes(o)&&(s?(log.info("- getting search"),O.base.fillSearchForm(s,o)):(log.info("- getting "+o),O.base._selectResource(o),O.base.updatePlaceholder(o),O.base.show(o)))))},addEventHandlers:function(e){O.base.logger("addEventHandlers()","adding event handlers");for(let o=0,s=e.length;o<s;o++){const{selectors:s,action:t,handler:n}=e[o];for(let e=0,o=s.length;e<o;e++){const o=s[e],l=o.name||o.id||`${o.nodeName}.${o.pathname.split("/").pop()}`;log.info(`- added ${n.name} to ${l}`),o.addEventListener(t,n.fn)}}},toggler:function(e){const o=e.target.name,s=document.getElementById(""+o);s.classList.contains("hidden-none")?(s.classList.add("visible-block"),s.classList.remove("hidden-none")):s.classList.contains("hidden-block")?(s.classList.add("visible-block"),s.classList.remove("hidden-block")):s.classList.contains("visible-block")&&(s.classList.add("hidden-block"),s.classList.remove("visible-block"))},openModal:function(e){const o=document.querySelectorAll("main section");for(let e=0,s=o.length;e<s;e++)O.base.hide(o[e].id);const s=e.target.href.split("/").pop().split(".").shift(),t=s+".html";history.pushState(null,null,t),O.base.show(s),e.stopPropagation(),e.preventDefault()},closeModal:function(e){let o=e.target.parentElement.id;O.base.hide(o),O.closedForMaintenance?o="closedForMaintenance":(o="index",O.base.show("searcher"));const s=O.closedForMaintenance?"index.html":o+".html";history.pushState(null,null,s),O.base.show(o),e.stopPropagation(),e.preventDefault()},show:function(e){log.info("showing "+e);const o=document.querySelector("#"+e);o.classList.add("visible-block"),o.classList.remove("hidden-none")},hide:function(e){log.info("hiding "+e);const o=document.querySelector("#"+e);o.classList.add("hidden-none"),o.classList.remove("visible-block")},formatSearchCriteria:function(e){const o=[];for(let s in e)if(!O.globals.notInSearchCriteria.includes(s)){const t=e[s];"q"===s?32===e[s].length?"treatments"===e.resource?o.push(`<span class="crit-key">treatmentId</span> is <span class="crit-val">${t}</span>`):"citations"===e.resource&&o.push(`<span class="crit-key">bibRefCitationId</span> is <span class="crit-val">${t}</span>`):o.push(`<span class="crit-val">${t}</span> is in the text`):o.push(`<span class="crit-key">${s}</span> is <span class="crit-val">${t}</span>`)}let s=o.length,t="";return 1===s?t+=o[0]:2===s?t+=o.join(" and "):s>2&&(s-=1,t+=o.slice(0,s).join(", ")+", and "+o[s]),t},toggleFigcaption:function(e){const o=document.querySelectorAll("figcaption");for(let e=0,s=o.length;e<s;e++)o[e].style.maxHeight=O.globals.closedFigcaptionHeight,o[e].style.overflow="hidden";const s=this.parentElement;s.style.maxHeight="100%",s.style.overflow="auto"},resourceFromUrl:function(e){const{hash:o,pathname:s,search:t}=e;O.base.logger("resourceFromUri()",e);let n=s.split("/").pop().split(".")[0];return""!==n&&"dev"!==n||(n="index"),log.info("- hash: "+o),log.info("- resource: "+n),log.info("- search: "+t),{hash:o,resource:n,search:t}},resourceFromForm:function(e){return document.querySelector("input[name=resource]:checked").value},resourceFromModalClick:function(e){const o=e.target.href;if(O.logger("resourceFromUri()",loc),log.info("- href: "+o),o)return o.split("/").pop().split(".").shift()},_selectResource:function(e){const o=document.querySelectorAll("#resourceChooser label"),s=document.querySelectorAll("#resourceChooser input");for(let t=0,n=o.length;t<n;t++)s[t].value===e?(s[t].checked=!0,o[t].classList.add("searchfocus")):(s[t].checked=!1,o[t].classList.remove("searchfocus"))},selectResource:function(e){const o=e.target.value;log.info("selected resource "+o),O.base._selectResource(o),O.base.updatePlaceholder(o)},updatePlaceholder:function(e){O.base.logger("updatePlaceholder()","updating placeholder"),O.base["num-of-records"][e]?O.base.setPlaceHolder({"num-of-records":O.base["num-of-records"][e],resource:e}):O.utils.getResource({resource:e,inputs:null,cb:O.base.setPlaceHolder})},setPlaceHolder:function(e){0==O.base["num-of-records"][e.resource]&&(O.base["num-of-records"][e.resource]=e["num-of-records"]);const o=`search ${e["num-of-records"]} ${e.resource}`;log.info(`placeholder: "${o}"`);const s=document.querySelector("input[name=q]");s.placeholder=o,s.focus()},fetchReceive:function(e){if(!e.ok)throw new Error("HTTP error, status = "+e.status);return e.json()},hls:function(e){const o=document.querySelectorAll("input[name=resource");for(let s=0,t=o.length;s<t;s++){const t=o[s],n=t.parentElement;t.value===e?(log.info(`setting checked of ${t.value} to true`),t.checked=!0,n.classList.add("searchfocus")):(t.checked=!1,n.classList.remove("searchfocus"))}},fillSearchForm:function(e,o){O.base.logger("fillSearchForm()","filling search form"),log.info("- search: "+e),log.info("- resource: "+o);const s=document.querySelector("#searcher"),t=new URLSearchParams(e);for(const[e,o]of t)if(O.globals.validQueryParams.includes(e)){const t=s.querySelectorAll(`input[name="${e}"]`);for(let e=0,s=t.length;e<s;e++){const s=t[e];"checkbox"===s.type?s.value===o&&(log.info(`setting checked of ${s.name} with value ${s.value} to 'true'`),s.checked=!0):s.value=o}}O.base.hls(o),O.utils.prepareQuery()}};
"undefined"!=typeof O&&"object"==typeof O||(O={}),O.utils={niceNumbers:function(e){return e>9?e:["No","One","Two","Three","Four","Five","Six","Seven","Eight","Nine"][e]},renderResults:function(e){const t=e.resource,n=e.template,s=document.querySelector(`#${t} div.result`),r=document.querySelector("#template-"+n).innerHTML,o={"template-pager":document.querySelector("#template-pager").innerHTML,"template-records-found":document.querySelector("#template-records-found").innerHTML,"template-kein-preview":document.querySelector("#template-kein-preview").innerHTML};s.innerHTML=Mustache.render(r,e,o);const i=document.querySelectorAll("figcaption > a");for(let e=0,t=i.length;e<t;e++)i[e].addEventListener("click",O.base.toggleFigcaption);return e.resource},prepareQuery:function(e){e&&(e.stopPropagation(),e.preventDefault());const t=document.querySelector("#searcher"),n=t.elements,s=t.querySelector("input[name=q]");if(""===s.value)return s.placeholder="c‘mon, enter something",s.classList.remove("nudge"),s.classList.add("warning"),s.focus(),!1;s.classList.remove("warning"),s.classList.add("nudge");const r={};for(let e=0,s=n.length;e<s;e++){const n=t[e];if(n.name){const e=n.name,t=n.value,s=n.type,o=n.checked;"checkbox"===s?o&&(e in r?r[e].push(t):r[e]=[t]):"radio"===s?o&&(r[e]=t):r[e]=t}}O.utils.getResource({resource:document.querySelector("input[name=resource]:checked").value,inputs:r,cb:O.utils.renderResults})},noThrob:function(e){const t=document.querySelectorAll("main section");for(let n=0,s=t.length;n<s;n++){const s=t[n].id;s!==e&&O.base.hide(s)}document.querySelector("#throbber").classList.add("nothrob")},getResource:function(e){const{resource:t,inputs:n,cb:s}=e,{z:r,b:o}=O.utils.inputs2uris(n,t);O.base.logger("getResource()","getting "+t),log.info("zenodeoUri: "+r),log.info("browserUri: "+o);let i;document.querySelector("#throbber").classList.remove("nothrob"),n&&(i=n?O.base.formatSearchCriteria(n):"",O.utils.waitMessage(t,i),history.pushState(null,null,o),O.base.show(t));try{fetch(r).then(O.base.fetchReceive).then((function(e){const s=e.item.result,r={resource:t,"num-of-records":s.count};if(n)if(r["search-criteria-text"]=i,32===n.q.length)"treatments"===s["search-criteria"].resource?r.template="treatment":"citations"===s["search-criteria"].resource&&(r.template="citation"),s.count&&(r.figures=s.records[0]);else if(r.template=r.resource,r.niceNumbers=O.utils.niceNumbers(s.count),s.count){if(r.successful=!0,1==s.count)r.shown="it is shown below";else if(s.count>1&&s.count<=O.globals.pageSize)r.shown=`one to ${r.niceNumbers} are shown below`;else{const e=O.utils.niceNumbers((n.page-1)*O.globals.pageSize+1),t=O.utils.niceNumbers(n.page*O.globals.pageSize);r.shown=`${e} to ${t} are shown below`}if(r.figures=function(e,t){let n=[];if("treatments"===e)for(let e=0,s=t.length;e<s;e++){const s=t[e];s.recId=s.treatmentId,n.push(s)}else if("images"===e)for(let e=0,s=t.length;e<s;e++){const s=t[e];n.push({title:s.metadata.title,creators:s.metadata.creators?s.metadata.creators.map((e=>e.name)):[],recId:s.id,zenodoRecord:`${O.globals.zenodoUri}/${s.id}`,description:s.metadata.description,doi:s.doi,img:!!s.links.thumbs,img10:s.links.thumbs?s.links.thumbs[10]:"",img50:s.links.thumbs?s.links.thumbs[50]:"",img100:s.links.thumbs?s.links.thumbs[100]:"",img250:s.links.thumbs?s.links.thumbs[250]:"",img750:s.links.thumbs?s.links.thumbs[750]:"",img1200:s.links.thumbs?s.links.thumbs[1200]:""})}else if("citations"===e)for(let e=0,s=t.length;e<s;e++){const s=t[e];s.recId=s.bibRefCitationId,n.push(s)}return n}(t,s.records),s.count>=O.globals.pageSize){const t=new URL(e.item._links._prev),n=new URL(e.item._links._next);r.prev=t.search,r.next=n.search,r.pager=!0}}else r.successful=!1,r.shown="",r.pager=!1,r.figures=[];return r})).then(s).then(O.utils.noThrob)}catch(e){log.error(e)}},waitMessage:function(e,t){const n=document.querySelector("#"+e);O.base.show(e);n.querySelector(".result").innerHTML=`<p id="records-found" class="records-found">Looking for ${e} where ${t}</p>`},inputs2uris:function(e,t){const n=[],s=[];for(const r in e)O.globals.zenodeoUriParams[t].includes(r)&&("q"===r?32==r.length?"treaments"===t?n.push("treatmentId="+e[r]):"citations"===t&&n.push("bibRefCitationId="+e[r]):n.push("q="+e[r]):n.push(`${r}=${e[r]}`)),O.globals.browserUriParams[t].includes(r)&&s.push(`${r}=${e[r]}`);const r=n.length?"?"+n.sort().join("&"):"";return{z:`${O.globals.zenodeoUri}/${t}${r}`,b:`${t}.html${s.length?"?"+s.join("&"):""}`}}};